...
let collectionData = []
let res = request('GET', `https://api.trello.com/1/board/${boardId}/lists?key=${apiKey}&token=${oauthToken}`)
...
Trello.board.searchCards(boardId).then((response) => {
	response.forEach(cardResponse => {
